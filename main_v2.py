### 121 EPOCH 기준으로 이어서 학습 ( learning rate 낮추고 
...
for kfold_idx, (index_kf_train, index_kf_validation) in enumerate(kf.split(labels, tds['len'])):
    ...
    model = RCNN_OCR()
    model.load_state_dict(torch.load('outputs_CUSTOM/0fold_126epoch_classifier.pt'))
    model = model.to(device)

    # criterion = torch.nn.CTCLoss().to(device)
    criterion = torch.nn.CrossEntropyLoss().to(device)
    optimizer = torch.optim.AdamW(params=model.parameters(), lr=0.00001, weight_decay=0.1)

    for ep in range(129, EPOCH):
        model.train()
        train_loss = []
        train_CER = []
